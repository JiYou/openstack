...
from monitor.api.views import backups as backup_views
from monitor.api import xmlutil
from monitor import backup as backupAPI
...
class CreateDeserializer(wsgi.MetadataXMLDeserializer):
    ...
    def _extract_backup(self, node):
        ...
        attributes = ['container', 'display_name',
                      'display_description', 'servicemanage_id']
...
class RestoreDeserializer(wsgi.MetadataXMLDeserializer):
    ...
    def _extract_restore(self, node):
        ...
        if restore_node.getAttribute('servicemanage_id'):
            restore['servicemanage_id'] = restore_node.getAttribute('servicemanage_id')
        return restore
...
class BackupsController(wsgi.Controller):
    ...
    # - whether requested servicemanage_id exists so we can return some errors
    #   immediately
    ...
    def create(self, req, body):
        ...
        LOG.audit(_("Creating backup of servicemanage %(servicemanage_id)s in container"
                    " %(container)s"), locals(), context=context)

        try:
            new_backup = self.backup_api.create(context, name, description,
                                                servicemanage_id, container)
        except exception.InvalidServiceManage as error:
            raise exc.HTTPBadRequest(explanation=unicode(error))
        except exception.ServiceManageNotFound as error:
            raise exc.HTTPNotFound(explanation=unicode(error))
    ...
    def restore(self, req, id, body):
        ...
        try:
            ...
            msg = _("Incorrect request body format")
            raise exc.HTTPBadRequest(explanation=msg)
        servicemanage_id = restore.get('servicemanage_id', None)

        LOG.audit(_("Restoring backup %(backup_id)s to servicemanage %(servicemanage_id)s"),
        ...
        try:
            ...
            raise exc.HTTPNotFound(explanation=unicode(error))
        except exception.ServiceManageSizeExceedsAvailableQuota as error:
            raise exc.HTTPRequestEntityTooLarge(
                explanation=error.message, headers={'Retry-After': 0})
        except exception.ServiceManageLimitExceeded as error:
            raise exc.HTTPRequestEntityTooLarge(
                explanation=error.message, headers={'Retry-After': 0})
...
class Backups(extensions.ExtensionDescriptor):
    ...
    name = 'Backups'
    alias = 'backups'
    namespace = 'http://docs.openstack.org/servicemanage/ext/backups/api/v1'
