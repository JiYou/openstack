...
URLs are mapped to "action" methods on "controller" classes in ``monitor/api/openstack/__init__/ApiRouter.__init__`` .
...
Controllers live in ``monitor/api/openstack``, and inherit from monitor.wsgi.Controller.

See ``monitor/api/openstack/servers.py`` for an example.
