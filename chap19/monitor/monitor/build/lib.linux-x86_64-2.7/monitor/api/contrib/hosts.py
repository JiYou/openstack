...
class HostController(object):
    ...
    def __init__(self):
        self.api = servicemanage_api.HostAPI()
        super(HostController, self).__init__()

    @wsgi.serializers(xml=HostIndexTemplate)
    def index(self, req):
        authorize(req.environ['monitor.context'])
    ...
    def show(self, req, id):
        ...
        try:
            host_ref = db.service_get_by_host_and_topic(context,
                                                        host,
                                                        FLAGS.servicemanage_topic)
        except exception.ServiceNotFound:
            raise webob.exc.HTTPNotFound(explanation=_("Host not found"))

        # Getting total available/used resource
        # TODO(jdg): Add summary info for Snapshots
        servicemanage_refs = db.servicemanage_get_all_by_host(context, host_ref['host'])
        (count, sum) = db.servicemanage_data_get_for_host(context,
                                                   host_ref['host'])

        snap_count_total = 0
        snap_sum_total = 0
        resources = [{'resource': {'host': host, 'project': '(total)',
                      'servicemanage_count': str(count),
                      'total_servicemanage_gb': str(sum),
                      'snapshot_count': str(snap_count_total),
                      'total_snapshot_gb': str(snap_sum_total)}}]

        project_ids = [v['project_id'] for v in servicemanage_refs]
        project_ids = list(set(project_ids))
        for project_id in project_ids:
            (count, sum) = db.servicemanage_data_get_for_project(context, project_id)
...
class Hosts(extensions.ExtensionDescriptor):
    ...
    name = "Hosts"
    alias = "os-hosts"
    namespace = "http://docs.openstack.org/servicemanage/ext/hosts/api/v1.1"
