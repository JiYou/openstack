...
if [ $coverage -eq 1 ]; then
    noseopts="$noseopts --with-coverage --cover-package=monitor"
fi
if [ $coverage_xml -eq 1 ]; then
    noseopts="$noseopts --with-xcoverage --cover-package=monitor --xcoverage-file=`pwd`/coverage.xml"
fi
...
# Files of interest
# NOTE(lzyeval): Avoid selecting monitor-api-paste.ini and monitor.conf in monitor/bin
...
srcfiles=`find monitor -type f -name "*.py" ! -path "monitor/openstack/common/*"`
srcfiles+=" `find bin -type f ! -name "monitor.conf*" ! -name "*api-paste.ini*"`"
...
if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='monitor/*' --omit='monitor/openstack/common/*' -d covhtml -i
fi
