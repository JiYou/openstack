...
"""monitor HACKING file compliance testing
...
IMPORT_EXCEPTIONS = ['sqlalchemy', 'migrate', 'monitor.db.sqlalchemy.session']
DOCSTRING_TRIPLE = ['"""', "'''"]
...
def monitor_todo_format(physical_line):
    ...
    monitor HACKING guide recommendation for TODO:
    ...
    if (pos != pos1 and pos2 >= 0 and pos2 < pos):
        return pos, "ENERGY N101: Use TODO(NAME)"


def monitor_except_format(logical_line):
    """Check for 'except:'.

    monitor HACKING guide recommends not using except:
    Do not write "except:", use "except Exception:" at the very least
    N201
    """
    if logical_line.startswith("except:"):
        yield 6, "ENERGY N201: no 'except:' at least use 'except Exception:'"


def monitor_except_format_assert(logical_line):
    """Check for 'assertRaises(Exception'.

    monitor HACKING guide recommends not using assertRaises(Exception...):
...
def monitor_one_import_per_line(logical_line):
    ...
    monitor HACKING guide recommends one import per line:
    Do not import more than one module per line

    Examples:
    BAD: from monitor.rpc.common import RemoteError, LOG
    N301
    """
    ...
    if (pos > -1 and (parts[0] == "import" or
        ...
        yield pos, "ENERGY N301: one import per line"
...
def monitor_import_module_only(logical_line):
    ...
    monitor HACKING guide recommends importing only modules:
...
def monitor_import_alphabetical(physical_line, line_number, lines):
    ...
    monitor HACKING guide recommendation for imports:
    ...
    if (len(split_line) in length and len(split_previous) in length and
        ...
        if split_line[1] < split_previous[1]:
            return (0,
                    "ENERGY N306: imports not in alphabetical order (%s, %s)"
...
def monitor_docstring_start_space(physical_line):
    ...
    monitor HACKING guide recommendation for docstring:
    ...
    if (pos != -1 and len(physical_line) > pos + 1):
        if (physical_line[pos + 3] == ' '):
            return (pos,
                    "ENERGY N401: one line docstring should not start with"
                    " a space")
...
def monitor_docstring_one_line(physical_line):
    ...
    monitor HACKING guide recommendation for one line docstring:
    ...
    if (pos != -1 and end and len(physical_line) > pos + 4):
        if (physical_line[-5] != '.' and physical_line):
            return pos, "ENERGY N402: one line docstring needs a period"
...
def monitor_docstring_multiline_end(physical_line):
    ...
    if (pos != -1 and len(physical_line) == pos):
        print physical_line
        if (physical_line[pos + 3] == ' '):
            return (pos, "ENERGY N403: multi line docstring end on new line")
        ...
        if token_type == tokenize.NAME and text == "_":
            ...
            if not format_string:
                raise LocalizationError(
                    start,
                    "ENERGY N701: Empty localization string")
            if token_type != tokenize.OP:
                raise LocalizationError(
                    start,
                    "ENERGY N701: Invalid localization call")
            if text != ")":
                if text == "%":
                    raise LocalizationError(
                        start,
                        "ENERGY N702: Formatting operation should be outside"
                        " of localization method call")
                elif text == "+":
                    raise LocalizationError(
                        start,
                        "ENERGY N702: Use bare string concatenation instead"
                        " of +")
                else:
                    raise LocalizationError(
                        start,
                        "ENERGY N702: Argument to _ must be just a string")
            ...
            if len(positional_specs) > 1:
                raise LocalizationError(
                    start,
                    "ENERGY N703: Multiple positional placeholders")
...
def add_monitor():
    ...
    Look for functions that start with monitor_  and have arguments
...
if __name__ == "__main__":
    #include monitor path
    sys.path.append(os.getcwd())
    #ENERGY error codes start with an N
