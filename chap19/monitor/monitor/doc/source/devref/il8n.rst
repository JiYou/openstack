...
====================
monitor uses `gettext <http://docs.python.org/library/gettext.html>`_ so that
...
in monitor/tests/test_localization.py.
...
In general, you shouldn't need to add these to any monitor files, since the lines
are present in ``monitor/__init__.py``. However, if this code is missing, it may
